...
%Crystal_size(1:n) = Crystal_size(1:n) - 2^(1/2)*S;
    ...
    Crystal_size_x      = linspace(0,1,Drob) * Crystal_size_max;
    ...
    for ii=first_cryst:n
        r = 1;
     while Crystal_size_x(r)<(Crystal_size(ii))
        r = r+1;
     end
...
set(axes_vel,'ylim',[-3 15]);
     ...
     %set(axes_raspr,'ylim',[-8 1]);
      ...
      plot(HRT_C_raspr_try(1,:)*1.e-6,HRT_C_raspr_try(2,:),Crystal_size_x,log(Crystal_size_y),'o');
