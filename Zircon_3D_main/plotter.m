...
fileID_4 = fopen(['4.dat'],'r');
fileID_5 = fopen(['5.dat'],'r');
fileID_6 = fopen(['6.dat'],'r');
fileID_7 = fopen(['7.dat'],'r');
...
S_left   = fscanf(fileID_4,formatSpec,size_time)';
S_right = fscanf(fileID_5,formatSpec,size_time)';
S_top  = fscanf(fileID_6,formatSpec,size_time)';
S_bot  = fscanf(fileID_7,formatSpec,size_time)';
...
Crystal_size(1:n) = ((S_right(1:n)-S_left(1:n)).^2 + (S_top(1:n)-S_bot(1:n)).^2).^(1/2);
...
shading interp;
 ...
 str_N_cryst           = {'N cryst=',num2str(n)};
      ...
      for j=1:n
          text((S_right(j) + S_right(j))/2/L_size,(S_top(j) + S_bot(j))/2/L_size,num2str(j), 'Color', 'black','FontSize', 10);
      end
